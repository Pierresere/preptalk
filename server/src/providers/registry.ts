...
export function createProviders(config: Config): ProviderMap {
  ...
  if (config.keys.openai !== undefined) map.set('openai', createOpenAiProvider(config.keys.openai))
  if (config.keys.gemini !== undefined) map.set('gemini', createGeminiProvider(config.keys.gemini))
  return map
}
