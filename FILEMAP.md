...
- `web/src/components/CompanyPanel.tsx` — splits `company` text on `## ` headings into cards (title + body rendered via `markdown.ts` `toHtml`, `dangerouslySetInnerHTML`); each card's title matched to a `SECTIONS` id (fr/en) gets a `company.rerun` button calling `onResearchSection(id)`; empty/whitespace `company` shows `company.empty` + a `.btn-primary` `company.researchAll` button; non-empty content also shows a top `.btn` `company.researchAll` button; all research buttons disabled while `busy`
