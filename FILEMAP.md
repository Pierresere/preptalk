...
- `web/src/components/PrepareScreen.tsx` — sub-tab row (`offer`/`resume`/`company`/`analysis`/`plan`/`documents`, via `useDossier`) plus a `nav.interview` button calling `onInterview`; offer/resume use `TextPanel`, company uses `CompanyPanel`, analysis uses `AnalysisPanel`, documents uses `DocumentsPanel`, plan uses `PlanEditor` (`onGenerate`→`generatePlan`, `onSave`→`savePlan`)
